...
Symmetries can be used to transform the hamiltonian matrix into block-diagonal form cf. \cite[56]{sand11} with each block belonging to states with a conserved quantum number, thereby reducing the basis dimension. In this first chapter, only the conservation of z-component of the total spin ($\hat{=}$ total magnetization) $m^z=\sum_{i=0}^{N-1} S_i^z$ is used:\\
...
\end{align}
meaning that $m^z$ is a conserved quantity. By constructing a basis with fixed magnetization, blocks with the same $S^z$ can be diagonalized independently, meaning that $S^z$ is a conserved quantity. By constructing a basis with fixed magnetization, blocks with the same $S^z$ can be considered independently.
...
\begin{align}
    \ket{a}=\hat{e}_a=(0,\dots,0,\underbrace{1}_{a},0,\dots,0)^T, a=1..M,     \label{eq:basismag}
\end{align}
